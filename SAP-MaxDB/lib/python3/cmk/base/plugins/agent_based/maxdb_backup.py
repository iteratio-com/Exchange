...
def parse_maxdb_backup(string_table):
...
def discover_maxdb_backup(section):
    for item in section:
        if item:
            yield Service(item=item)


def check_maxdb_backup(item, params, section):
    ...
    if not data.get('error_jobs'):
        ...
        for item in error_jobs:
            job_infos = data.get(item)
            txt += f"{item} - started: {job_infos.get('start')} - duration: {render.timespan(job_infos.get('duration'))}"
            txt += f" - Type: {job_infos.get('type')} - Age : {render.timespan(job_infos.get('age'))}"
            txt += f" - MSG: {job_infos.get('error_msg')}\n"
        yield Result(state=State.WARN,
                     summary=f"There are {len(data.get('error_jobs'))} (see long output)",
                     details=txt)
