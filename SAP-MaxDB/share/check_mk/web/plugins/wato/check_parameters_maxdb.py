...
def _parameter_valuespec_maxdb_database():
    return Dictionary(
        elements=max_db_size_elements + [
            ('badindexes',
             MonitoringState(title=_("Set here the Status for Bad Indexes"), help=_("Set the criticality here for a number of bad indexes greater than zero"),default_value=1)),
            ('badvolumes',
             MonitoringState(title=_("Set here the Status for Bad Volumes"), help=_("Set the criticality here for a number of bad volumes greater than zero"),default_value=2)),
        ],
        ...
        elements=[
            ('autosave',
             MonitoringState(
                 ...
                 help=
                 _("The AutoSave options is importand for some usechases. If this Option is not 'ON' the criticality can set here."
                  ),
                 default_value=0)),
        ],
        optional_keys=[],
