...
from kplus_labels.utils import get_additional_external_labels
...
def discover_interfaces(  # pylint: disable=too-many-branches
    ...
    for item, discovered_params, index, index_as_item, labels in pre_inventory:
        ...
        if additional := get_additional_external_labels(host_name=host_name(), item_name=new_item):
            if labels:
    ...
    *,
    ...
    for attributes in matching_attributes:
        ...
        member_info = MemberInfo(
            ...
            admin_status_name=(
                None if attributes.admin_status is None else statename(attributes.admin_status)
            ),
        )
