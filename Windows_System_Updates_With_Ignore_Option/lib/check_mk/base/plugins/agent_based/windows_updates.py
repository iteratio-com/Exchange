...
from .agent_based_api.v1 import (Result, Service, State, check_levels,
                                 register, render)
...
def check_windows_updates(params: Mapping[str, Any], section: Section) -> CheckResult:
    ...
    updates_not_ignored = []
    ignored_updates = params.get("ignored_updates", [])

    for line in imp_update:
        if not any(ignored_string in line for ignored_string in ignored_updates):
            updates_not_ignored.append(line)
            
    yield from check_levels(
        len(updates_not_ignored),
    ...
    if len(ignored_updates) > 0:
        yield Result(state=State.OK, notice=f"Beware! Ignored updates containing these strings in name: {'; '.join(ignored_updates)}")

    if updates_not_ignored:
        yield Result(state=State.OK, notice=f"Found updates: ({'; '.join(updates_not_ignored)})")
